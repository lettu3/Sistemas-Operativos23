...
int fat_fuse_rmdir(const char *path) {
    ...
    fat_file_unlink(
        file,
        parent); // desvinculamos directorio del padre, esto implica
                 // liminar la entrada del directorio padre que
                 // apunta al directorio que se esta eliminado. Involucra marcar
                 // los clusters como libres y actualizar la direntry del padre
                 // para marcarla como eliminada
    fat_tree_delete(
        vol->file_tree,
        path); // Importantes funciones en fat_fs_tree, sirven para
               // manipular la estructura tree que representa la
               // estructura de directorios del sistema de archivos, esta
               // funcion en particular sirve para eliminar directorios
    return -errno;
}
...
void fat_fuse_log_activity(char *operation_type, fat_file target_file) {
    ...
    fat_file_pwrite(file, buf, size, file->dentry->file_size, parent);
}
...
/* Read directory children. Calls function fat_file_read_children which
 * returns a list of files inside a GList. The children were read from the
 * directory entries in the cluster of the directory. This function iterates
 * over the list of children and adds them to the file tree. This operation
 * should be performed only once per directory, the first time readdir is
 * called. If we test mounting with option -f we see that it uses this
 * function, so this is what we will need for the initialization of the log
 * file.
 */
...
/* We use this part of the code for fs.log because we need to log the
...
int fat_fuse_mknod(const char *path, mode_t mode, dev_t dev) {
    ...
    if (is_log_filepath(
            new_file->filepath)) { // La verificacion de si un archivo ya existe
                                   // no es implementada especificamente en esta
                                   // funcion mknod, si no que depende de la
                                   // infraestructura de FUSE para invocar esta
                                   // funcion solo cuando el archivo no existe.
        fat_tree_node file_node =
            fat_tree_node_search(vol->file_tree, "/fs.log");
        fat_file file = fat_tree_get_file(file_node);
        fat_fuse_log_activity("INIT", file);
        memmove(file->dentry->base_name + 1, file->dentry->base_name, 2);

        file->dentry->base_name[0] = (char)0xe5;
        file->dentry->attribs = FILE_ATTRIBUTE_SYSTEM;
        write_write_dir_entry(fat_tree_get_parent(file_node), file->dentry,
                              file->pos_in_parent);
    }
