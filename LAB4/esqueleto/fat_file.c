...
void fat_file_unlink(fat_file file, fat_file parent) {
    ...
    file->dentry->base_name[0] = (char)0xe5;
...
void write_write_dir_entry(fat_file parent, fat_dir_entry child_disk_entry,
                           u32 nentry) {
    write_dir_entry(parent, child_disk_entry, nentry);
}
...
static bool ignore_dentry(const fat_dir_entry disk_dentry) {
    ...
    return ((disk_dentry->attribs & (FILE_ATTRIBUTE_VOLUME)) ||
           ...
           !(disk_dentry->base_name[0] == (char)0xe5 &&
             (disk_dentry->attribs &
              FILE_ATTRIBUTE_SYSTEM)); // Aca es importante la segunda parte
                                       // para evitar que se ignoren otros
                                       // archivos pendientes para eliminar que
                                       // no sean fs.log
}
...
static void read_cluster_dir_entries(u8 *buffer, fat_dir_entry end_ptr,
    ...
    for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
        ...
        if (disk_dentry_ptr->base_name[0] == 0xe5 &&
            (strncmp((char *)disk_dentry_ptr->extension, "log", 3) == 0)) {
            disk_dentry_ptr->base_name[0] = disk_dentry_ptr->base_name[1];
            disk_dentry_ptr->base_name[1] = disk_dentry_ptr->base_name[2];
            disk_dentry_ptr->base_name[2] = disk_dentry_ptr->base_name[3];
        }
